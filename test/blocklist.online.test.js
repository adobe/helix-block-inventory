...
describe('Blocklist Tests (online)', () => {
  it('BlockList fetches inventory', async () => {
    const list = await (await new BlockList('https://main--express-website--adobe.hlx.live/documentation/block-inventory').fetch()).parse();
    assert.ok(list);
    assert.deepEqual(list.blocks.pop(), {
      name: 'link-list',
